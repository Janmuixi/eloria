...
<template>
  <div>
    ...
    <div v-else>
      ...
      <div v-else-if="evt.paymentStatus !== 'paid'" class="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-6 flex items-start gap-4">
        <svg class="w-6 h-6 text-amber-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <div class="flex-1">
          <p class="font-semibold text-amber-900">{{ $t('eventDetail.paymentPending') }}</p>
          <p class="text-sm text-amber-700 mt-1">{{ $t('eventDetail.paymentPendingDescription') }}</p>
        </div>
        <NuxtLinkLocale
          :to="`/dashboard/events/new?step=5&eventId=${eventId}`"
          class="flex-shrink-0 px-4 py-2 bg-champagne-500 text-white rounded-full text-sm font-medium hover:bg-champagne-600 transition-colors"
        >
