...
CREATE TABLE `__new_companions` (
	...
	`allergies` text,
	`created_at` text DEFAULT '2026-05-26T08:25:03.597Z',
	FOREIGN KEY (`guest_id`) REFERENCES `guests`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
INSERT INTO `__new_companions`("id", "guest_id", "position", "name", "attending", "allergies", "created_at") SELECT "id", "guest_id", "position", "name", "attending", "allergies", "created_at" FROM `companions`;--> statement-breakpoint
...
CREATE TABLE `__new_events` (
	...
	`design_locked` integer DEFAULT false NOT NULL,
...
--> statement-breakpoint
INSERT INTO `__new_events`("id", "user_id", "title", "couple_name_1", "couple_name_2", "date", "venue", "venue_address", "venue_map_url", "description", "template_id", "invitation_type", "custom_image_path", "customization", "tier_id", "payment_status", "design_locked", "stripe_payment_id", "language", "allergies_enabled", "slug", "created_at") SELECT "id", "user_id", "title", "couple_name_1", "couple_name_2", "date", "venue", "venue_address", "venue_map_url", "description", "template_id", "invitation_type", "custom_image_path", "customization", "tier_id", "payment_status", CASE WHEN "payment_status" IN ('paid','locked') THEN 1 ELSE 0 END, "stripe_payment_id", "language", "allergies_enabled", "slug", "created_at" FROM `events`;--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO `__new_guest_menu_choices`("id", "guest_id", "course_id", "option_id", "companion_id", "created_at") SELECT "id", "guest_id", "course_id", "option_id", "companion_id", "created_at" FROM `guest_menu_choices`;--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO `__new_guests`("id", "event_id", "name", "email", "phone", "rsvp_status", "companions_allowed", "token", "email_sent_at", "email_opened_at", "allergies", "created_at") SELECT "id", "event_id", "name", "email", "phone", "rsvp_status", "companions_allowed", "token", "email_sent_at", "email_opened_at", "allergies", "created_at" FROM `guests`;--> statement-breakpoint
...
CREATE TABLE `__new_tiers` (
	...
	`created_at` text DEFAULT '2026-05-26T08:25:03.596Z'
);
--> statement-breakpoint
